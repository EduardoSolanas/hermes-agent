...
class TestScanCronPrompt:
    ...
    def test_invisible_unicode_stripped_before_scan(self):
        assert _sanitize_cron_prompt("normal text\u200b") == "normal text"
        assert _sanitize_cron_prompt("zero\ufeffwidth") == "zerowidth"
...
class TestUnifiedCronjobTool:
    ...
    def test_create_strips_invisible_unicode_from_stored_prompt(self):
        created = json.loads(
            cronjob(
                action="create",
                prompt="Check\u200b server status",
        ...
        assert created["success"] is True
        assert created["job"]["prompt_preview"] == "Check server status"

    def test_update_strips_invisible_unicode_from_stored_prompt(self):
        created = json.loads(cronjob(action="create", prompt="Check", schedule="every 1h"))
        job_id = created["job_id"]

        updated = json.loads(cronjob(action="update", job_id=job_id, prompt="New\ufeff prompt"))
        assert updated["success"] is True
        assert updated["job"]["prompt_preview"] == "New prompt"
