...
async def _put_invalid(page: Page, fallbacks: list[dict]) -> dict:
    return await page.evaluate("""async (fb) => {
        const token = window.__HERMES_SESSION_TOKEN__;
        const r = await fetch('/api/model/fallbacks', {
            method: 'PUT',
...
class TestLayout:
    ...
    async def test_add_opens_picker(self, page: Page):
        ...
        assert visible, "Picker should open after clicking Add"
...
class TestAdd:
    ...
    async def test_add_via_picker_then_reorder(self, page: Page):
        ...
        last_idx = count - 1
        await page.locator(f"[data-testid='fallback-move-up-{last_idx}']").click()
...
class TestRemove:
    ...
    async def test_remove_auto_saves(self, page: Page):
        ...
        await _goto(page)
        await page.locator("[data-testid='fallback-remove-0']").click()
        await page.wait_for_timeout(2000)
        config = _parse_yaml(await _get_raw_yaml(page))
        fallbacks = config.get("fallback_providers", [])
    ...
    async def test_remove_preserves_order(self, page: Page):
        ...
        await _goto(page)
        await page.locator("[data-testid='fallback-remove-1']").click()
...
class TestReorder:
    ...
    async def test_move_down_changes_ui_order(self, page: Page):
        ...
        await _goto(page)
        await page.locator("[data-testid='fallback-move-down-0']").click()
        await page.wait_for_timeout(500)
        order = await _ui_order(page)
        assert order[0] == "second · mb"
        assert order[1] == "first · ma"
    ...
    async def test_reorder_auto_saves_to_config(self, page: Page):
        ...
        await _goto(page)
        await page.locator("[data-testid='fallback-move-up-2']").click()
        await page.wait_for_timeout(1000)
        config = _parse_yaml(await _get_raw_yaml(page))
        fallbacks = config["fallback_providers"]
    ...
    async def test_multiple_reorders(self, page: Page):
        ...
        for idx in range(3, 0, -1):
            await page.locator(f"[data-testid='fallback-move-up-{idx}']").click()
            await page.wait_for_timeout(300)
        order = await _ui_order(page)
        assert order[0] == "d · md"
        await page.wait_for_timeout(1000)
