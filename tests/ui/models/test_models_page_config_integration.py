...
# =====================================================================
# UI interaction tests — exercise the ModelPickerDialog
# =====================================================================
...
class TestAddFallbackViaUI:
    ...
    @pytest.mark.asyncio
    async def test_ui_add_provider_persists_via_api(self, page: Page):
        """Full UI flow: Add → picker → select provider → verify auto-save persists to config.yaml."""
        ...
        # Step 6: Click the Save button in the picker dialog
        confirm_button = page.get_by_role("button", name="Save", exact=True)
        ...
        # Step 8: Verify config.yaml has the new fallback (auto-saved)
        yaml_text = await _get_raw_config_yaml(page)
        config = _parse_yaml_config(yaml_text)
        fallbacks = config.get("fallback_providers", [])
        assert len(fallbacks) >= 1, "Config should have at least one fallback"
    ...
    @pytest.mark.asyncio
    async def test_ui_add_clears_legacy_key(self, page: Page):
        """Adding via UI picker should clear legacy fallback_model."""
        await _save_fallbacks(page, [])
        await _go_to_fallback_chain(page)
...
async def _add_via_picker(page: Page, provider_slug: str, model_name: str) -> None:
    """Full UI flow: open picker → select provider → select model → confirm."""
