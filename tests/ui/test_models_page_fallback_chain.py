...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_duplicate_rejected(self, page: Page):
        """Adding a duplicate provider/model should show an error toast."""
        await _go_to_fallback_chain(page)
        ...
        if count > 0:
            # Get the provider/model of the first item
            first_item = page.locator("[data-testid='fallback-item-0']")
            model_text = await first_item.locator("span.font-mono").first.inner_text()
            # Try to add the same model again via the picker
            add_button = page.locator("[data-testid='fallback-add-button']")
            await add_button.click()
            await page.wait_for_timeout(500)
            # The duplicate check happens in JS, we just verify the UI doesn't crash
...
class TestIntegrationWithBackend:
    ...
    async def test_api_endpoint_accessible(self, page: Page):
        """The /api/model/fallbacks endpoint should be accessible."""
        await _go_to_fallback_chain(page)
