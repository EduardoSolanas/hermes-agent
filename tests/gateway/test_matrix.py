...
class TestMatrixAutoTrustDevices:
    def test_auto_trust_verifies_unverified_devices(self):
        ...
        # device_store is dict-like: user_id → {device_id → device}
        fake_store = {
            "@alice:ex.org": {"DEVICE_A": device_a, "DEVICE_B": device_b},
            "@bob:ex.org": {"DEVICE_C": device_c},
        }
        fake_client.device_store = MagicMock()
        fake_client.device_store.__iter__ = MagicMock(return_value=iter(fake_store.keys()))
        fake_client.device_store.__getitem__ = MagicMock(side_effect=fake_store.__getitem__)
    ...
    def test_auto_trust_skips_own_device(self):
        ...
        fake_store = {"@bot:ex.org": {"MY_DEVICE": own_device}}
        fake_client.device_store = MagicMock()
        fake_client.device_store.__iter__ = MagicMock(return_value=iter(fake_store.keys()))
        fake_client.device_store.__getitem__ = MagicMock(side_effect=fake_store.__getitem__)
